...
document.querySelector('.bl-scroll').addEventListener('click', function () {

    const scrollAmount = 245 + 40;
    const maxTranslateX = -scrollAmount * 6;

    let currentTranslateX = parseFloat(getComputedStyle(document.querySelector('.bl-svg-list')).transform.split(',')[4]) || 0;
    ...
    document.querySelector('.bl-svg-list').style.transform = `translateX(${newTranslateX}px)`;
});
